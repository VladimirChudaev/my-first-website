          ...
          <div className="space-y-12">
            {visibleNews.length > 0 ? (
              visibleNews.map((item) => {
                ...
                const imageUrl = media ? `${baseUrl}/storage/v1/object/public/${media.bucket}/${media.path}` : null;
                const createdAt = item.created_at ? new Date(item.created_at).toLocaleDateString('ru-RU') : '';
                    ...
                    <div className="flex flex-col md:flex-row gap-8 items-center md:items-start bg-white rounded-3xl p-2 transition-all">
                      <div className="w-full md:w-[450px] aspect-[16/10] shrink-0 rounded-2xl overflow-hidden bg-gray-100 shadow-sm">
                        {imageUrl ? (
                          ...
                          <div className="w-full h-full flex items-center justify-center text-gray-300 text-xs uppercase tracking-widest">Нет фото</div>
                        )}
                      </div>
                      <div className="flex-1 py-2">
                        <h2 className="text-2xl md:text-3xl font-light mb-4 leading-tight group-hover:text-gray-600 transition-colors">{item.title}</h2>
                        <p className="text-gray-500 text-sm md:text-base leading-relaxed line-clamp-3 mb-6">{plainTextBody}</p>
                        <div className="text-[11px] font-mono text-gray-400 uppercase tracking-[0.2em]">{createdAt}</div>
                      </div>
                    </div>
                  </Link>
                );
