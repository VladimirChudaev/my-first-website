...
export default function SiteHeader() {
  ...
  const socialLinks = [
    { icon: <FaTelegramPlane />, href: "t.me/VandTAgency" },
    { icon: <FaVk />, href: "https://vk.com/club230590987?from=groups" },
      ...
      <div className="container mx-auto flex flex-col md:flex-row items-center justify-between">
        ...
        <nav className="flex flex-wrap gap-4 text-sm mb-4 md:mb-0">
          {navigationItems.map((item, index) => (
            <a key={index} href={item.href} className="hover:text-gray-700 transition-colors whitespace-nowrap">
              {item.label}
            </a>
          ))}
        </nav>
        ...
        <div className="flex gap-4 text-lg">
          {socialLinks.map((link, index) => (
            <a key={index} href={link.href} className="hover:text-gray-700 transition-colors" target="_blank" rel="noopener noreferrer">
              {link.icon}
            </a>
          ))}
        </div>
